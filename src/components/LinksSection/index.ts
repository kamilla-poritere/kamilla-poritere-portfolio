import { t } from "../../modules/i18n";
import "./style.css";

interface Link {
  nameKey: string;
  url: string;
  icon: string;
  color: string;
}

const links: Link[] = [
  {
    nameKey: "links.items.0.name",
    url: "https://github.com/kamilla-poritere",
    icon: `<svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/></svg>`,
    color: "var(--text)",
  },
  {
    nameKey: "links.items.1.name",
    url: "https://linkedin.com/in/kamilla-poritere-177721198",
    icon: `<svg viewBox="0 0 24 24" fill="currentColor"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/></svg>`,
    color: "#0077b5",
  },
  {
    nameKey: "links.items.2.name",
    url: "https://internet-presence.co.uk/Kamilla",
    icon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="2" y1="12" x2="22" y2="12"></line><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"></path></svg>`,
    color: "var(--primary)",
  },
  {
    nameKey: "links.items.3.name",
    url: "mailto:kamilla.poriter@gmail.com",
    icon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"></path><polyline points="22,6 12,13 2,6"></polyline></svg>`,
    color: "var(--accent-red)",
  },
];

const ARROW_ICON = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="5" y1="12" x2="19" y2="12"></line><polyline points="12 5 19 12 12 19"></polyline></svg>`;

function createTitle(): HTMLHeadingElement {
  const title = document.createElement("h2");
  title.className = "links__title";
  title.dataset.i18n = "links.title";
  title.textContent = t("links.title");
  return title;
}

function createIcon(link: Link): HTMLSpanElement {
  const iconWrapper = document.createElement("span");
  iconWrapper.className = "links__icon";
  iconWrapper.style.color = link.color;
  iconWrapper.innerHTML = link.icon;
  const svg = iconWrapper.querySelector("svg");
  if (svg) {
    svg.setAttribute("aria-hidden", "true");
    svg.setAttribute("focusable", "false");
  }
  return iconWrapper;
}

function createArrow(): HTMLSpanElement {
  const arrow = document.createElement("span");
  arrow.className = "links__arrow";
  arrow.innerHTML = ARROW_ICON;
  const svg = arrow.querySelector("svg");
  if (svg) {
    svg.setAttribute("aria-hidden", "true");
    svg.setAttribute("focusable", "false");
  }
  return arrow;
}

function createLinkText(link: Link): HTMLSpanElement {
  const textWrapper = document.createElement("span");
  textWrapper.className = "links__text";
  textWrapper.dataset.i18n = link.nameKey;
  textWrapper.textContent = t(link.nameKey);
  return textWrapper;
}

function createLinkItem(link: Link, index: number): HTMLAnchorElement {
  const a = document.createElement("a");
  a.href = link.url;
  a.className = "links__item";
  a.target = "_blank";
  a.rel = "noopener noreferrer external";
  a.style.animationDelay = `${index * 0.1}s`;
  a.setAttribute("aria-label", t(link.nameKey));
  a.appendChild(createIcon(link));
  a.appendChild(createLinkText(link));
  a.appendChild(createArrow());
  return a;
}

function createLinksContainer(): HTMLDivElement {
  const container = document.createElement("div");
  container.className = "links__container";
  container.setAttribute("role", "list");
  links.forEach((link, i) => {
    const item = createLinkItem(link, i);
    item.setAttribute("role", "listitem");
    container.appendChild(item);
  });
  return container;
}

function createSection(): HTMLElement {
  const section = document.createElement("section");
  section.className = "links";
  section.appendChild(createTitle());
  section.appendChild(createLinksContainer());
  return section;
}

// Render + i18n refresh
let langBound = false;

function renderLinksSection(): void {
  const container = document.querySelector<HTMLDivElement>("#links-section");
  if (!container) return;

  container.innerHTML = "";
  container.appendChild(createSection());

  if (!langBound) {
    langBound = true;
    window.addEventListener("langchange", () => {
      renderLinksSection();
    });
  }
}

renderLinksSection();
